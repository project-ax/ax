...
**Lesson:** When verifying an HMAC of a request body, compute the HMAC over the raw body string/buffer BEFORE any `JSON.parse` → `JSON.stringify` round-trip. Round-tripping re-orders keys, changes whitespace, re-escapes unicode, etc., and breaks byte-exact comparison. Read the body into a `string`, HMAC that string, THEN parse for validation. The order of operations inside the handler is: (1) read raw body with size cap, (2) verify signature on raw, (3) parse + validate. Doing signature verification BEFORE body validation also prevents attackers from probing Zod error shapes.
