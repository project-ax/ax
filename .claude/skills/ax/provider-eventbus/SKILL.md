...
| `nats`      | `nats.ts`       | NATS JetStream | Dual-subject routing; listener eviction at capacity |
...
- Uses JetStream with dual-subject routing: `events.global` + `events.{requestId}`.
...
- `src/providers/eventbus/nats.ts` — NATS JetStream implementation
