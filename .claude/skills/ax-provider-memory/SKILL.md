...
- **Parallel LIKE + embedding search:** `items-store.ts` and `provider.ts` run text-based LIKE search and embedding-based vector search in parallel, merging results. This improves both query performance and recall compared to sequential fallback.
