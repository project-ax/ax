...
tests/
  agent/
    ...
    nats-ipc-client.test.ts  # NATS IPC client
    nats-warm-pod-flow.test.ts # NATS warm pod flow
  ...
  host/
    ...
    nats-ipc-handler.test.ts      # NATS IPC handler
    ...
    ipc-handlers/
      ...
      sandbox-tools.test.ts      # Sandbox tool routing (sandbox_approve/sandbox_result)
  ...
  providers/
    ...
    credentials/
    skills/
      database.test.ts  # Database-backed skills provider
    ...
    sandbox/
      ...
      warm-pool-client.test.ts # Warm pool client
    scheduler/
    storage/
...
- **NATS / K8s tests**: `nats-session-protocol.test.ts`, `nats-ipc-handler.test.ts`, `nats-llm-proxy.test.ts`, `nats-ipc-client.test.ts`, `nats-warm-pod-flow.test.ts` -- NATS messaging and K8s pod sandbox
- **Warm pool tests**: `providers/sandbox/warm-pool-client.test.ts`, `providers/sandbox/k8s-warm-pool.test.ts` -- K8s warm pool lifecycle
- **IPC handler tests**: `host/ipc-handlers/` -- per-handler tests for sandbox-tools, workspace, workspace-file, identity, governance, memory, orchestration, skills-install, image, llm-events, web
...
- **Workspace provider tests**: `providers/workspace/` -- gcs, local, none, shared, gcs-transport
- **Sandbox provider tests**: `providers/sandbox/` -- apple, docker, subprocess, k8s, k8s-warm-pool, warm-pool-client, canonical-paths, utils
