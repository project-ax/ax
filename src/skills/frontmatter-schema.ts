...
const McpServerSchema = z
  .object({
    ...
    /** Optional glob patterns (minimatch syntax) selecting which of the
     *  server's advertised tools to surface in the unified tool catalog.
     *  When omitted, every tool the server lists is included. Matched
     *  case-sensitively against the MCP tool's bare `name` (not the
     *  `mcp_<skill>_<name>` catalog name). Skill authors use this to pin
     *  the interface surface to known-safe tools — e.g. `include: ['list_*', 'get_*']`
     *  to exclude destructive mutations. Applied before `exclude`. */
    include: z.array(z.string().min(1)).optional(),
    /** Optional glob patterns (minimatch syntax) selecting tools to omit
     *  from the catalog after `include` has been applied. Useful for a
     *  "mostly everything, but not these few" posture. Matched the same
     *  way as `include`. */
